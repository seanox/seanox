...
if (typeof(DataSource) == "undefined") {
    ...
    DataSource.transform = function(xml, style, partial) {
        ...
        var nodes = result.querySelectorAll("*[escaping]");
        nodes.forEach(function(node, index, array) {
            if ((node.getAttribute("escaping") || "on").match(/^yes|on|true|1$/i))
    ...
    DataSource.collect = function(variants) {
        ...
        collection.forEach(function(entry, index, array) {
            ...
            root.documentElement.appendChild(DataSource.fetch(entry).documentElement);
        });
